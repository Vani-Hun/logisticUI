...
export default function ServiceAll() {
  ...
  //     if (params.id) {
  //       const getservice = async () => {
  //         const res = await axios.get(`${END_POINT}/service/${params}`);
  //         console.log(res);
  //         const { data } = res.data;
  //         console.log(data);
  //         setServices(data);
  ...
  //       };
  //       getservice();
  //       const getquote = async () => {get(
  //           `${END_POINT}/quote/service/${params}`
  //         );
  ...
  //           `${END_POINT}/feature/service/${params}`
  //         );
  ...
  //         const res = await axios.get(
  //           `${END_POINT}/participant/service/${params}`
  //         );
  ...
  console.log("serviceAll", services);

  var serviceNumber = 0;
  {
    services.map((service, index) => {
      const result = params.id.replace('-', ' ');
      console.log(service, result);
      if (service.name.toLowerCase() === result.toLowerCase()) {
        serviceNumber = index
      }
      const countpar = document.querySelectorAll('.wrapper_objects_service');

      if(services[serviceNumber].participants.length < 3){
        if(services[serviceNumber].participants.length === 2){
          countpar[0].className = "wrapper_objects_service lg:grid  lg:grid-cols-2 gap-[20px] sm:block"
        }
        else{
          countpar[0].className = "wrapper_objects_service lg:grid  lg:grid-cols-1 gap-[20px] sm:block"
        }
      }
      else{
        countpar[0].className = "wrapper_objects_service lg:grid  lg:grid-cols-3 gap-[20px] sm:block"
      }
  ...
  return (
    <section id="layout-content">
      {services.length > 0 && services[serviceNumber].banner &&
      <div className="h-full lg:h-[610px] w-full relative">
        <img src={services[serviceNumber].banner} className="lg:absolute lg:right-0 lg:top-0 w-full h-full lg:w-auto object-cover" style={{ width: '180%', }}  onLoad={()=>setload(true)} alt="express" />
        <div class="container mx-auto h-full flex items-center">
          <div class="w-full h-auto lg:w-[540px] relative">
            <img src={services[serviceNumber].sub_banner} className="w-full h-full object-cover hidden lg:block"/>
            <div class="lg:absolute left-[50%] lg:top-[50%] lg:translate-x-[-50%] lg:translate-y-[-50%] lg:w-full lg:h-full lg:py-6 px-4 lg:px-0">
              <h5 className="mt-6 lg:mt-0 text-[#D60009] Montserrat-Bold font-sans text-[24px] lg:text-[36px] aos-init font-[700]">
                {services[serviceNumber].sub_detail}
              </h5>
              <span className="block my-6 lg:my-4 text-justify lg:text-left text-[18px] font-light">
                {services[serviceNumber].target}
              </span>
              <Link to='/tu-van/dang-ki-tu-van'>
                <button class="flex lg:inline-flex justify-center items-center bg-[#FF0000] rounded-[2px] text-white w-full lg:w-[215px]  h-[56px] mt-8 lg:mt-4">
                  <img
                  ...
                  />
                  <span class="ml-2 Montserrat-Bold font-black text-xl">Đăng ký tư vấn</span>
                </button>
              </Link>
            </div>
          </div>
      ...
      <div className="container mx-auto lg:pt-[80px] lg:pb-[58px] lg:mt-0" style={{ marginTop: 75 }}>
        <div className="wrapper_description_service px-4 grid lg:px-0 md:grid-cols-1 sm:grid-cols-1  lg:grid-cols-[700px_minmax(400px,_1fr)_200px] gap-[100px]  ">
          <div className="wrapper_description_service_detail lg:mt-0 grid lg:grid-cols-2 gap-[70px] md:grid-cols-1 sm:grid-cols-1" >
            {services.length > 0 && services[serviceNumber].features.map((feature, index) => {
              return <div class="flex item-start  " >
                <img
                  class="w-[48px] h-[48px] object-cover"
                  src={`${services[serviceNumber].features[index].logo}`}
                  alt="J&amp;T Express Việt Nam - Chuyển phát tiêu chuẩn"
                />
                <div class="flex flex-col ml-4" >
                  {/* <h5 class="mb-2 text-[#D60009] text-[20px] Montserrat-Bold font-bold"> */}
                  <h5 class="mb-2 text-[#D60009] text-[24px] Montserrat-Bold ">
                    <b>
                      {services[serviceNumber].features[index].name}
                    </b>
                  </h5>
                  <span class="text-justify aos-init aos-animate text-[17px]">
                    {services[serviceNumber].features[index].detail}
                  </span>
                </div>
              </div>
            })
      ...
      <div className="lg:bg-[#F4F4F4] lg:pt-[63px] lg:pb-[58px]">
        <div className="container mx-auto mt-10 lg:mt-0 px-4 lg:px-0 mb-10 lg:mb-0 ">
          ...
          >
            ...
            {services.length > 0 && services[serviceNumber].tip}
          </span>
          <div className="w-[27px] h-[3px] bg-[#f5c736] mx-auto mb-8"></div>
          <div className="wrapper_objects_service lg:grid  lg:grid-cols-3 gap-[20px] sm:block">
            ...
            {services.length > 0 && services[serviceNumber].participants.map((participant, index) => {
              return (
              <div class="h-[315px] lg:h-[375px] relative rounded-[10px] overflow-hidden sm:mb-[20px]">
                <img
                  src={`${services[serviceNumber].participants[index].banner}`}
                  className="w-full h-full object-cover "
                  alt="J&amp;T Express Việt Nam - Chuyển phát tiêu chuẩn"
                />
                <div class="object-service-detail absolute top-[50%] translate-y-[-50%] left-[10%] text-white w-[170px] lg:w-[320px]">
                  <span class="block Montserrat-Bold mb-3 text-[20px] font-bold">
                    {services[serviceNumber].participants[index].name}
                  </span>
                  <span
                    data-aos="fade-up"
                    data-aos-duration="1000"
                  ...
                  >
                    {services[serviceNumber].participants[index].description}
                  </span>
                </div>
              </div>)
            })
      ...
      <div className="">
        <CarouselWrapper
          ...
          autoplaySpeed={3500}
        >
          {services.length > 0 && services[serviceNumber].quotes.map((quote, index) => {
            return <div className="relative">
              <img
                // src="https://jtexpress.vn/themes/jtexpress/assets/images/slider-tuyen-dung.png"
                src={services[serviceNumber]?.quote_banner}
                className="w-full h-[380px] md:h-[500px] object-cover bg-black"
              ...
              <div className="absolute top-0 bottom-0 left-0 right-0">
                <div className="flex items-center justify-center flex-col  mt-[60px] md:mt-[100px] ">
                  <img
                    src={services[serviceNumber].quotes[index].avatar}
                    alt="avatar"
                  ...
                  <div className="mx-8 sm:mx-[70px] md:mx-[130px] lg:mx-[320px] mt-[12px]">
                    <div className="text-white font-semibold text-lg text-center sm:text-xl md:text-2xl preventselect">
                      {services[serviceNumber].quotes[index].quote}
                    </div>
                  </div>
                  <h1 className="text-white font-bold text-lg mt-[14px] preventselect">
                    {services[serviceNumber].quotes[index].name}
                  </h1>
                  <div className="text-white text-base sm:text-lg preventselect">
                    {services[serviceNumber].quotes[index].description}
                  </div>
                </div>
              </div>
            </div>
      ...
      {services.length && services[serviceNumber].video &&
      <div style={{backgroundImage: `url(${services[serviceNumber].bottom_banner})`}} className="bg-no-repeat bg-cover lg:bg-[#F4F4F4] lg:pt-[63px] lg:pb-[58px]">
        <div className="container mx-auto mt-10 lg:mt-0 px-4 lg:px-0 mb-10 lg:mb-0 ">
          <h5
            className="font-black text-4xl md:text-5xl text-[#161D25] text-center aos-init"
            data-aos="fade-right"
          >
            Video giới thiệu dịch vụ
          </h5>
          <span
            className="block text-center mt-10 mb-4 w-full lg:w-[578px] mx-auto aos-init text-base"
            data-aos="zoom-in"
          >
            {services.length > 0 && services[serviceNumber].introduce}
          </span>
          <div className=" flex justify-center p-3">
              {parse(services[serviceNumber].video)}
          </div>
        </div>
      </div>}
