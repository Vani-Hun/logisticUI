...
export default function BuuCuc() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${END_POINT}/post-office/search`, {
        params:{
      ...
      <div className="flex items-center flex-col lg:flex-row gap-[24px]">
        <div className="w-full h-[43px] lg:w-1/3">
          ...
          {!isValid && !provinceSelected && (
            <div className="text-[#F0B90B] ">{t('Vui lòng chọn tỉnh/thành phố')}</div>
          )}
        </div>
        <div className="w-full  h-[43px] lg:w-1/3 ">
          <select
          ...
          {!isValid && !districtSelected && (
            <div className="text-[#F0B90B] ">{t('Vui lòng chọn quận/huyện')}</div>
          )}
        </div>
              ...
              >
                <div className="flex flex-row justify-between gap-x-1">
                  ...
                  >
                    ...
                    <span className=" text-lg text-inherit font-semibold min-w-[131px]">{t('Tìm đường đi')}</span>
                  </a>
                </div>
                <div className="flex items-stretch">
                  <IoLocationOutline className="w-4 h-4 inline-block mr-2" />
                  <span className="font-semibold">{warehouse.address}-{warehouse.district}-{warehouse.province} </span>
                </div>
              </div>
            ))}
          </div>
