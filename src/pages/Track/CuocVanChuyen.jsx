...
export default function CuocVanChuyen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // });
      console.log(`${wardFrom.name},${districtFrom.name},${provinceFrom.name}`)
      console.log(`${wardTo.name},${districtTo.name},${provinceTo.name}`)
      // console.log(weight)
      
      // console.log(serviceName.toLowerCase())
      const info = await axios.post(`${END_POINT}/shippingcost/shipping-cost`, {
        name1: `${wardFrom.name},${districtFrom.name},${provinceFrom.name}`,
        name2: `${wardTo.name},${districtTo.name},${provinceTo.name}`,
        weight: weight,
        shippingtype: serviceName,
      });
      console.log(info.data)
      setPrice(info.data.data.shippingcost);
      ...
      <div className="flex-col">
        <div className="w-full">
          <span className="mb-4 inline-block font-bold text-xl text-[#F0B90B]">
            {t('Gửi từ')}: <span className="text-red-500"> * </span>
          </span>
        ...
        <div className="lg:w-[15%] w-full my-2 ml-2">
          <div>
            ...
            <div className="my-2">
              <span className="mb-4 inline-block font-bold text-xl text-[#F0B90B]">
                {t('Dịch vụ')}: <span className="text-red-500"> * </span>
              </span>
              <div className="w-full h-[43px]">
                <select
                  tabIndex={-1}
