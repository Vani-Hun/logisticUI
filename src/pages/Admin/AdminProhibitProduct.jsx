...
export default function AdminProhibitProduct() {
  ...
  const acceptEditNewProhibit = async (e) => {
    ...
    const tableData = e.target.parentElement.parentElement.parentElement.querySelectorAll('input');
    image.append("image", imgFile);
    // data.append("name", tableData[0].value);
    // data.append("detail", tableData[1].value);

    try {
      const res = await axios({
        url: `${END_POINT}/admin/prohibited-product/${idItem}`,
        method: "put",
        headers: { 'x-access-token': `${accessToken}` },
        data: {name: tableData[0].value, detail: tableData[1].value},
      })
    } catch (error) {
      console.log(error);
    }
    if(imgFile){
      try {
        const res1 = await axios({
          url: `${END_POINT}/admin/prohibited-product/image/${idItem}`,
          method: "put",
          headers: { 'x-access-token': `${accessToken}` },
          data: image,
        })
