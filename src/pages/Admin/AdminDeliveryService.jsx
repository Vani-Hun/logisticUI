...
function AdminDeliveryService() {
  ...
  const [isAddBannerLogoVisible, setIsAddBannerLogoVisible] = useState(false);
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      console.log("Call API Service",response.data.service);
  ...
  const handleClickBannerLogo = (record) => {
    setIsAddBannerLogoVisible(true);
    const [dataEdit] = data.filter((ele) => ele.name === record.name);
    setDataForEdit(dataEdit);
  };
  ...
  return (
    <div>
      ...
      {isAddBannerLogoVisible && (
        <AddBannerLogo
          onClose={() => setIsAddBannerLogoVisible(false)}
          data={dataForEdit}
          refetchData={() => fetchData(params)}
        />
      )}
