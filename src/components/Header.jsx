...
const Header = () => {
  const { user, aboutUs, logoutHandle } = useContext(MainContext);
  ...
  return (
    <div>
      <SearchBar />
      <div className="fixed top-8 bg-white inset-x-0 h-[65px] z-20">
        <div className=" lg:static flex justify-around items-center h-full px-4 lg:px-0 container mx-auto text-sm ">
          ...
          <div className="hidden sm:block">
            <Link to="/">
              <img src={aboutUs.logo} className="max-h-[200px] max-w-[200px]" alt="logo-TKTL" />
            </Link>
          </div>
          <ul className="hidden lg:flex h-full justify-center items-center m-0">
            ...
            <div className="group hover:bg-yellow-200 rounded-md">
              <Link to="/dich-vu">
                <span
                  className={`inline-flex items-center px-4 py-2 ${comparePath("/dich-vu") || comparePath("chuyen-phat")
                    } cursor-pointer`}
                >
                  {t("Dịch vụ")}
                  <FaChevronDown className="h-4 w-4 pl-[6px]" />
              ...
              <ul className="hidden group-hover:block absolute bg-white rounded-lg z-10 border shadow-lg animate-up">
                {services.map((service) => (
                  <li>
                    <Link
                      to={"dich-vu/" + ChangeToSlug(service.name)}
